...
long map(long x, long in_min, long in_max, long out_min, long out_max) {
  re
	turn (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}
...
uint16_t adcValue[2]; //ADC
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
  	if(adcValue[0]<1800)//x<1800
  	{
  		setMotorStart(1,map(adcValue[0], 0, 1799, 999, 0));
  	}
  	else if(adcValue[0]>2200)//x>2200
  	{
  		setMotorStart(0,map(adcValue[1], 2201, 4095, 0, 999));
  	}
  	else//1800<=x<=2200
  	{
  		setMotorStop();
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, Motor1_Pin|Motor2_Pin, GPIO_PIN_RESET);
